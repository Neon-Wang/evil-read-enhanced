...
Production reruns are idempotent across partial failures. If a run is interrupted after Collections PDFs are archived, the next run restores same-day imported entries from `collections/logs/import_manifest.json` instead of dropping them from the daily report. Zotero Run JavaScript UI failures leave the generated reconcile scripts in `zotero/library/logs/` and continue with the workspace mirror. If PDF2zh cannot translate a collection PDF, Start My Day writes a small audited fallback `.zh.pdf` explaining the failed translation and including extracted source-text preview so packaging and review stay traceable.
...
Zotero plugin parity is part of the production contract, not a manual afterthought. Before a new machine runs `start-my-day`, install the required Zotero plugins documented in `docs/zotero-environment.md` and verify them with `tools/zotero_env_audit.py`; the run is not green while required plugins are missing.
